...
MAX_LIN     = 0.15       # [07-15 저전력] 0.18→0.15: 전원 브라운아웃(모터 뚝 정지→전도) 완화.
                         # 전원 정비(배터리 만충·아두이노 전원 분리) 후 0.18 복귀 검토
MAX_LIN_REV = 0.08
MAX_ANG     = 0.85       # [07-15 저전력] 1.0→0.85: 제자리 회전이 전류 피크 최대 구간
ALLOW_REVERSE = True

FRONT_STOP_M = 0.20      # [v4_1_5] 0.25→0.20: 실차 체감 반영 (유령포인트 2점 필터 전제)

# [v4_1_5] v4_1_1 + 최소 회피. 07-14 반복 실험의 결론:
#   쾌적함을 죽인 건 재획득 게이트·정면 킥이었고 순수 척력 회피가 아님 (#53).
#   인식/재획득은 v4_1_1 그대로 두고, 아래 3가지만 추가:
#   ① front_blocked 유령포인트 수정 ② 자기구조물 마스킹 ③ 보수적 척력 회피
# [v4_1_6] 코너 케이스: 사람이 화면 경계에 잘린 채(상반신 일부) 잡히면 중심오차가
# 커서 전진이 0이 되고, 가장자리 방위 라이다 콘이 코너 구조물을 사람 거리로 오인해
# 데드밴드 정지 → 로봇이 코너를 향해 전진하지 못하던 문제 (07-14 실차 확인).
# bbox가 좌/우 경계에 닿은 프레임에 한해서만 두 가지 완화 적용:
EDGE_CLIP_MIN_HOLD = 0.35   # 전진 하한 35% 보장 (중앙 감속으로 0이 되지 않게)
...
AVOID_W_MAX    = 0.6     # 회피 조향 상한
SELF_MASK_DIST  = 0.35   # 기동 20스캔 중 15회+ 이 거리 이내 각도 = 카트 프레임 → 제외
SELF_MASK_SCANS = 20
...
ACC_LIN_UP   = 0.18   # [07-15 저전력] 0.25→0.18: 가속 돌입 전류(inrush) 완화
...
if _ROS2_OK:
    ...
    class RobotController(Node):
        ...
        def __init__(self, topic: str = "cmd_vel", esp_ip: str = None, pi_ip: str = None,
            ...
            self._self_mask = None     # 자기 구조물 각도 마스크
            self._mask_scans = []
            self.start_x = 0.0
            self.start_y = 0.0
            self.start_yaw = 0.0
        ...
        def _scan_cb(self, msg):
            ...
            # [v4_1_5] 자기 구조물 마스킹: 기동 직후 정지 스캔으로 카트 프레임 각도 학습
            if self._self_mask is None:
                ...
                if len(self._mask_scans) >= SELF_MASK_SCANS:
                    ...
                    self.get_logger().info(
                        f"[mask] 자기 구조물 마스킹: {sum(mask)}칸 (~{deg:.0f}도) 제외")
        ...
        def front_blocked(self) -> bool:
            ...
            [v4_1_5] range_min 미만 유령 포인트(반사 0.01~0.11m)가 '막힘'으로 오판되던
            버그 수정: 유효 범위 필터 + 근접 2점 이상 요구 (단일 노이즈 무시)."""
        ...
        def _lidar_dist_cm(self, cx_norm, half_deg=15.0):
            """bbox 중심 방위각 ±half_deg도 라이다 최솟값(cm). 유효 값 없으면 None."""
            scan = self.latest_scan
            if scan is None or not scan.ranges:
                return None
            ...
            if center is None:
                return None
            n = len(scan.ranges)
            half = max(1, int(round(math.radians(half_deg) / scan.angle_increment)))
            valid = []
        ...
        def compute(self, bbox, frame_w, frame_h, is_tracking):
            ...
            # 라이다 우선, 없으면 bbox 폭 핀홀 폴백 (경계 잘림 시 콘 축소 — 코너 오인 감소)
            lidar_cm = self._lidar_dist_cm(
                cx_norm, half_deg=EDGE_CLIP_CONE_DEG if edge_clip else 15.0)
            ...
            # [v4_1_5] 보수적 척력 회피 — 마스킹 완료 후에만, 추종 대상 방위 ±15° 제외.
            # 정면 킥·측면 차단 없음: 살짝 비켜가는 용도, 완전 차단은 front_blocked 담당.
            avoidance_w = 0.0
            scan = self.latest_scan
            ...
            if v > 0:
                hold = max(0.0, 1.0 - abs(err_c) * CENTER_HOLD_GAIN)
                if edge_clip:
                    # [v4_1_6] 경계 잘림 프레임은 전진 하한 보장 — 회전만으로는 코너의
                    # 사람을 중앙에 못 모으므로, 전진하며 코너를 지나야 시야가 열림
                    hold = max(hold, EDGE_CLIP_MIN_HOLD)
                v *= hold
            elif edge_clip and v == 0.0 and dist_cm >= DIST_NEAR_CM:
                # [v4_1_6] 코너 정지↔전진 반복 제거: 콘을 좁혀도 구조물이 간헐적으로
                # 읽혀 데드밴드 정지가 끼어들면 멈칫거림 발생 → 저속 전진 유지.
                # (진짜 근접은 front_blocked 0.20m·척력 회피가 그대로 방어)
                v = EDGE_CLIP_CREEP
            self.last_edge_clip = edge_clip   # 디버그 로그·검출 가속용
            # [v4_1_5] 회피 중 완만 감속 (최저 40% — 멈칫 느낌 방지)
            if v > 0 and abs(avoidance_w) > 0.1:
                v *= max(0.4, 1.0 - abs(avoidance_w))
            return v, w
            ...
            if follower.state == "FOLLOW":
                ...
                if frame_count % 3 == 0:        # 로그는 ≈5-7Hz로 샘플링
                    ...
                    DBG.log("cmd", v=round(follower.last_v, 3), w=round(follower.last_w, 3),
                            ...
                            edge=getattr(follower, "last_edge_clip", False),
...
def main() -> int:
    ...
    speak_ip = args.speak_ip or args.pi_ip   # [v4_1_5] 음성 목적지: 키오스크 지정 가능
