#!/usr/bin/env python3
"""
ros_person_follower_nav2_v3 — 등록 사용자 인식 + 추종 + Nav2 원점 복귀 (분산 처리판)
...
실행(VM)
  ...
  python3 ros_person_follower_nav2_v3.py --pi-ip 192.168.0.23 --register
...
class DebugLog:
    ...
    def __init__(self, enabled: bool = False, ver: str = "v3"):
        self.f = None
        self.path = None
        if enabled:
...
if _ROS2_OK:
    ...
    class RobotController(Node):
        """추종 속도 발행 + Nav2 원점 복귀 + 상태(FOLLOW/RETURN/IDLE) 관리."""
        ...
        def _lidar_dist_cm(self, cx_norm):
            ...
            angle_idx = int(round(math.degrees(bearing_rad))) % 360
            valid = []
...
def register(cam, yolo, reid, user_id, grace_sec: float = 5.0):
...
def main() -> int:
    ...
    global DBG
    DBG = DebugLog(enabled=not args.no_debug_log, ver="v3")
    if DBG.path:
        print(f"[debug] 로그 기록: {DBG.path} (분석: python3 debug/analyze_debug.py)")
    DBG.log("start", imgsz=args.imgsz, reid_model="x1_0",
