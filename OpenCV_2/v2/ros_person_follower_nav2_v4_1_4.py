...
FRONT_STOP_M = 0.20      # [v4_1_3] 0.25→0.20: 실차 체감상 정지가 일렀음 (유령포인트 2점 필터 전제)

# [장애물 우선 회피] 우선순위: ①인식(카메라, 항상 동작) ②장애물 회피 ③추종 조향.
# 장애물이 회피 범위에 들면 추종 조향을 절반으로 줄이고 척력 조향을 우선 합성 + 감속.
# ※ 07-14 오전 시도는 카트 프레임(자기 구조물)을 장애물로 오인해 전면 정지 → 롤백.
#    이번엔 기동 직후 자기 구조물 각도를 자동 마스킹한 뒤에만 회피를 켠다.
AVOID_RANGE_M  = 0.55    # 전방 ±50° 이 거리 이내 장애물부터 척력 조향
AVOID_GAIN     = 0.45    # 장애물 점당 척력 계수
AVOID_W_MAX    = 0.8     # 회피 조향 상한 (rad/s)
# [v4_1_2] 2인 테스트(07-14 15:47)에서 끼어든 타인이 ReID 0.655~0.709로 구제 통과 →
# 구제 임계 상향 + 수확 가드 강화 + 구제 재획득 직후 수확 동결(프로필 오염 차단).
# ※ light_features.py는 v4_1_1(검증본)과 공유하므로 여기 로컬 상수로만 강화.
RESCUE_REID_STRICT   = 0.68   # ReID 하한은 유지하되, 아래 시간창·방위 게이트가 주 방어선
HARVEST_REID_STRICT  = 0.80   # LF.HARVEST_REID_MIN(0.75) 대체
HARVEST_FREEZE_SEC   = 30.0   # 구제로 재획득한 뒤 이 시간 동안 수확 금지 (타인 오염 방지)
# [v4_1_3] 임계값으로는 유실 본인(0.6~0.7)과 타인(0.65~0.8)의 ReID가 겹쳐 분리 불가
# (07-14 590초 로그: RESCUE 31회 중 다수가 타인). 물리 논리로 전환:
#   가림(occlusion) = 같은 방향에서 수 초 내 재등장 / 끼어들기 = 다른 시간·방향.
# → 구제는 "유실 후 4초 이내 + 마지막 목격 방위 ±45°" 안에서만 허용.
#   (이 4초 동안 로봇은 회전하지 않으므로 방위 비교가 유효함)
RESCUE_WINDOW_SEC    = 4.0    # 유실 후 이 시간 안에서만 ReID 구제 허용
RESCUE_BEARING_DEG   = 45.0   # 마지막 목격 방위와 이 각도 이내 후보만 구제

# 자기 구조물 마스킹: 기동 직후 스캔 20회 중 15회 이상 0.35m 이내로 잡히는 각도
# = 카트 프레임/기둥 → 이후 모든 근접 판정(회피·전방정지·사람거리)에서 제외
SELF_MASK_DIST  = 0.35
SELF_MASK_SCANS = 20

# [v4_1_4] 막힘 우회(detour): 반응형 회피는 코너/매대처럼 사람·장애물 방향이 겹치면
# 국소최소로 정지만 가능 (07-14 로그: 장애물성 정지 28%, 최장 20.9초).
# → 막힘이 지속되면 사람의 map 좌표를 Nav2에 단발 goal로 맡겨 우회 후 반응형 복귀.
#   (#47 방법 A의 연속 리플랜과 달리 막힐 때 1회만 발사 — 주행 끊김 없음)
DETOUR_BLOCK_SEC    = 3.0   # 전방 막힘+사람 보임이 이 시간 지속되면 우회 시작
DETOUR_TIMEOUT_SEC  = 12.0  # 우회 최대 시간 — 초과 시 취소하고 반응형 복귀
DETOUR_GOAL_BACK_M  = 0.6   # goal은 사람 위치에서 로봇 쪽으로 이만큼 앞 지점
DETOUR_COOLDOWN_SEC = 5.0   # 우회 종료 후 재우회 금지 시간 (goal 연사 방지)

# 유실 탐색: 등록자 놓치면 제자리서 좌우 교대 저속 회전(v=0)으로 재탐색
SEARCH_ANG         = 0.45   # [v4_1_2] 0.20→0.45: 0.2는 11도/초라 정지로 보임 (유실 후 안 찾는다는 오해)
SEARCH_HALF_PERIOD = 7.0    # 좌우 180도(π rad): π/0.45 ≈ 7.0초
SEARCH_START_DELAY = 4.0    # [v4_1_3] RESCUE_WINDOW_SEC와 일치 — 구제 창 동안은 정지 유지
...
if _ROS2_OK:
    ...
    class RobotController(Node):
        ...
        def __init__(self, topic: str = "cmd_vel", esp_ip: str = None, pi_ip: str = None,
            ...
            self._self_mask = None     # 자기 구조물 각도 마스크 (True = 무시할 인덱스)
            self._mask_scans = []      # 마스크 산출용 초기 스캔 표본
            self.start_x = 0.0
            self.start_y = 0.0
            self.start_yaw = 0.0
            ...
            self._blocked_since = None     # 전방 막힘 지속 시작 시각 (장애물 음성 안내용)
            self.cur_x = 0.0               # [v4_1_4] AMCL 현재 위치 (우회 goal 삼각법용)
            self.cur_y = 0.0
            self.cur_yaw = 0.0
            self.has_cur_pose = False
            self._detour_t = 0.0           # 우회 시작 시각
            self._detour_end_t = 0.0       # 우회 종료 시각 (쿨다운 기준)
        ...
        def _amcl_cb(self, msg):
            p0 = msg.pose.pose
            self.cur_x = p0.position.x
            self.cur_y = p0.position.y
            self.cur_yaw = _quat_to_yaw(p0.orientation.x, p0.orientation.y,
                                        p0.orientation.z, p0.orientation.w)
            self.has_cur_pose = True
            if not self.has_start_pose:
                p = msg.pose.pose
        ...
        def _scan_cb(self, msg):
            ...
            # [자기 구조물 마스킹] 기동 직후 정지 상태 스캔으로 카트 프레임 각도 자동 학습
            if self._self_mask is None:
                self._mask_scans.append(list(msg.ranges))
                if len(self._mask_scans) >= SELF_MASK_SCANS:
                    n = len(msg.ranges)
                    same = [s for s in self._mask_scans if len(s) == n]
                    need = int(len(same) * 0.75)
                    mask = []
                    for i in range(n):
                        near = sum(1 for s in same
                                   if msg.range_min < s[i] < SELF_MASK_DIST
                                   and math.isfinite(s[i]))
                        mask.append(near >= need)
                    self._self_mask = mask
                    self._mask_scans = []
                    deg = sum(mask) * 360.0 / n
                    self.get_logger().info(
                        f"[mask] 자기 구조물 마스킹 완료: {sum(mask)}칸 (~{deg:.0f}도) 제외")
                    DBG.log("self_mask", cells=sum(mask), deg=round(deg, 1))

        def _masked(self, idx) -> bool:
            """해당 스캔 인덱스가 자기 구조물 각도면 True."""
            m = self._self_mask
            return m is not None and idx < len(m) and m[idx]
        ...
        def front_blocked(self) -> bool:
            ...
            # [v4_1_2] range_min 미만 유령 포인트(반사/간섭 0.01~0.11m)가 "막힘"으로
            # 오판되던 버그 수정: 유효 범위 필터 + 근접 2점 이상 요구(단일 노이즈 무시).
            # (07-14 로그: 사람 65~76cm 정상 측정 중 front_blocked 오판으로 23% 완전 정지)
            vals = [scan.ranges[i] for i in idxs
                    if not self._masked(i)
                    and scan.range_min < scan.ranges[i] < scan.range_max
                    and math.isfinite(scan.ranges[i])]
            return sum(1 for r in vals if r < FRONT_STOP_M) >= 2
        ...
        def _sector_min(self, deg_from, deg_to):
            """방위 구간(도, 정면 0·CCW+ = 좌측 양수) 내 라이다 최솟값(m). 마스크 제외."""
            scan = self.latest_scan
            ...
            while True:
                r = scan.ranges[i]
                if (not self._masked(i)
                        and scan.range_min < r < scan.range_max and math.isfinite(r)):
                    vals.append(r)
                if i == i1:
                    break
                i = (i + 1) % n
        ...
        def compute(self, bbox, frame_w, frame_h, is_tracking):
            ...
            # ── [우선순위 ② 장애물 회피] 마스킹 완료 후에만 동작.
            #    전방 ±50°/0.55m 이내 장애물(자기 구조물·추종 대상 제외)에 척력 조향.
            #    회피가 활성화되면 추종 조향(③)은 절반으로 양보하고 전진 감속.
            avoidance_w = 0.0
            scan = self.latest_scan
            if (self._self_mask is not None and scan is not None
                    and scan.ranges and scan.angle_increment > 0):
                n = len(scan.ranges)
                ...
                for idx in range(n):
                    if self._masked(idx):
                        continue
                    r = scan.ranges[idx]
                    if not (scan.range_min < r < AVOID_RANGE_M and math.isfinite(r)):
                        continue
                    ...
                    if person_b is not None and abs(ang - person_b) < math.radians(15.0):
                        continue   # 추종 대상(①에서 인식된 사람)은 회피 대상 아님
                    weight = (AVOID_RANGE_M - r) / AVOID_RANGE_M
                    avoidance_w += -math.copysign(1.0, ang) * weight * AVOID_GAIN
                avoidance_w = _clamp(avoidance_w, -AVOID_W_MAX, AVOID_W_MAX)
                # [v4_1_2] 정면(±20°) 장애물은 좌우 척력이 상쇄돼 avoidance_w가 약해짐 →
                # 안 피하고 직진하다 front_blocked로 정지만 하던 문제. 정면이 막혔는데
                # 척력이 약하면 좌우 여유 공간을 비교해 빈 쪽으로 결정적 조향.
                if abs(avoidance_w) < 0.15:
                    fmin = None
                    for idx in range(n):
                        if self._masked(idx):
                            continue
                        r = scan.ranges[idx]
                        if not (scan.range_min < r < 0.45 and math.isfinite(r)):
                            continue
                        ang = scan.angle_min + idx * scan.angle_increment
                        ang = math.atan2(math.sin(ang), math.cos(ang))
                        if abs(ang) >= math.radians(20.0):
                            continue
                        if person_b is not None and abs(ang - person_b) < math.radians(15.0):
                            continue
                        if fmin is None or r < fmin:
                            fmin = r
                    if fmin is not None:
                        lf = self._sector_min(15, 55) or 2.0
                        rf = self._sector_min(-55, -15) or 2.0
                        avoidance_w = (AVOID_W_MAX * 0.5) if lf >= rf else -(AVOID_W_MAX * 0.5)
            if abs(avoidance_w) > 0.1:
                # 장애물 우선: 추종 조향 50% 양보 + 척력 합성
                w = _clamp(0.5 * w + avoidance_w, -MAX_ANG, MAX_ANG)

            # 중앙 우선 주행: 사람이 중앙에서 벗어날수록 전진을 감속해
            # 회전으로 먼저 중앙에 모은다 (가장자리로 밀려 시야 이탈하는 것 방지)
            if v > 0:
                v *= max(0.0, 1.0 - abs(err_c) * CENTER_HOLD_GAIN)
            # 회피 중 전진 감속 (장애물 가까울수록 더 감속, 완전 정지는 안 함 —
            # 정지는 ③이 아니라 front_blocked(전방 ±15°/0.25m) 최종 안전선이 담당)
            if v > 0 and abs(avoidance_w) > 0.1:
                v *= max(0.25, 1.0 - abs(avoidance_w) * 1.2)
            return v, w
        ...
        def send_velocity(self, v, w):
            ...
            if v > 0 and self.front_blocked():       # 안전: 전방 막히면 전진 취소
                v = 0.0
                # [v4_1_3] 장애물 앞 2초 이상 정지 지속 시 음성 안내 — 어설픈 회피 대신
                # 사용자가 경로를 바꾸도록 유도 (스피커 speak()에 5초 쿨다운 내장)
                if self._blocked_since is None:
                    self._blocked_since = time.time()
                elif time.time() - self._blocked_since > 2.0:
                    speak_on_pi(self.pi_ip, "앞에 장애물이 있어요. 조금만 돌아가 주세요.")
            else:
        ...
        def start_detour(self):
            """[v4_1_4] 막힘 우회: 사람의 map 좌표(방위+라이다 거리+현재 pose 삼각법)로
            Nav2 단발 goal 발사. 도착/재정렬/타임아웃 시 반응형 추종으로 복귀."""
            if (not self.has_cur_pose or self._last_bearing is None
                    or self.last_dist_cm <= 0):
                return False
            d = self.last_dist_cm / 100.0
            theta = self.cur_yaw + self._last_bearing
            gd = max(0.3, d - DETOUR_GOAL_BACK_M)
            gx = self.cur_x + gd * math.cos(theta)
            gy = self.cur_y + gd * math.sin(theta)
            self.state = "DETOUR"
            self._detour_t = time.time()
            self._blocked_since = None
            self.get_logger().info(
                f"[detour] 막힘 지속 → Nav2 우회 goal ({gx:.2f}, {gy:.2f}) 사람거리 {d:.2f}m")
            DBG.log("detour", gx=round(gx, 2), gy=round(gy, 2), dist_m=round(d, 2),
                    bearing_deg=round(math.degrees(self._last_bearing), 1))
            self.send_nav_goal(gx, gy, theta)
            return True
        ...
        def _nav_done_cb(self, future):
            ...
            if self.state == "DETOUR":      # [v4_1_4] 우회 도착 → 반응형 추종 복귀
                self.get_logger().info("[detour] 우회 goal 종료 → 반응형 추종 복귀")
                self.state = "FOLLOW"
                self._detour_end_t = time.time()
                return
...
def run_tracking(cam, yolo, reid, face, profile, use_face=True, follower=None,
    ...
    last_rescue_t = 0.0  # 마지막 구제 재획득 시각 — 직후 HARVEST_FREEZE_SEC 동안 수확 금지
    ...
    while True:
        ...
        # [v4_1_4] DETOUR는 "인식 유지 + 주행만 Nav2" 상태 — FOLLOW와 동급으로 취급
        ACTIVE_STATES = ("FOLLOW", "DETOUR")
        if cur_state not in ACTIVE_STATES and prev_state in ACTIVE_STATES:
            tracker.reset(); kcf.deinit(); reg_det_bbox = None
            last_bboxes, last_scores = [], {}
            if follower is not None:
                follower.reset_search()
        elif cur_state in ACTIVE_STATES and prev_state not in ACTIVE_STATES:
            last_seen_t = time.time()   # 재진입 → 유실 탐색 대기 타이머 리셋
        prev_state = cur_state
        follow_active = (cur_state in ACTIVE_STATES)
        ...
        if fresh:
            ...
            if (not matched and bb is not None
                    ...
                    and detail.get("reid", 0) >= RESCUE_REID_STRICT
                    and detail.get("color", 0) >= LF.COLOR_FLOOR
                    # [v4_1_3] 시간창: 유실 직후에만 (끼어들기는 대개 그 뒤에 발생)
                    and time.time() - last_seen_t < RESCUE_WINDOW_SEC):
                # [v4_1_3] 방위 게이트: 마지막 목격 방향 ±45° 밖에서 나타난 후보는 타인 간주
                ok_bearing = True
                if follower is not None and follower._last_bearing is not None:
                    cxg = (bb[0] + bb[2]) / 2.0
                    cand_b = (0.5 - cxg / max(1, frame.shape[1])) * math.radians(70.0)
                    ok_bearing = abs(cand_b - follower._last_bearing) < math.radians(RESCUE_BEARING_DEG)
                if ok_bearing:
                    matched = rescued = True
                    last_rescue_t = time.time()   # 수확 동결 타이머 시작
            if matched:
                ...
                if (tracker.status == "tracking" and not rescued
                        and detail.get("reid", 0) >= HARVEST_REID_STRICT
                        and t_m - last_rescue_t >= HARVEST_FREEZE_SEC
        ...
        # ── 주행 ──
        if follower is not None:
            # [v4_1_4] 막힘 우회 트리거: 사람은 보이는데 전방 막힘이 3초 지속 → Nav2 우회
            if (follower.state == "FOLLOW" and tracker.is_tracking
                    and draw_bbox is not None
                    and follower._blocked_since is not None
                    and time.time() - follower._blocked_since > DETOUR_BLOCK_SEC
                    and time.time() - follower._detour_end_t > DETOUR_COOLDOWN_SEC):
                follower.start_detour()
            # [v4_1_4] 우회 관리: 사람이 다시 정면(±30%)이고 전방이 뚫리면 조기 복귀
            if follower.state == "DETOUR":
                recentered = (draw_bbox is not None
                              and abs(((draw_bbox[0] + draw_bbox[2]) / 2 - w_f / 2)
                                      / (w_f / 2)) < 0.3
                              and not follower.front_blocked())
                if recentered or time.time() - follower._detour_t > DETOUR_TIMEOUT_SEC:
                    follower.cancel_nav()
                    follower.state = "FOLLOW"
                    follower._detour_end_t = time.time()
