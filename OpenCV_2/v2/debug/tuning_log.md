...
| 상수 | 원본 | 1차 | 2차(현재) | 3차 | 의도 |
|---|---|---|---|---|---|
| `MATCH_THRESHOLD` | 0.72 | 0.74 | 0.74 | | 진입 엄격 |
| `SEARCH_MATCH_THR` | 0.70 | 0.65 | 0.70 (복귀) | | 재획득 (0.65는 불안정해서 되돌림) |
| `KEEP_THRESHOLD` | 0.62 | 0.56 | 0.60 | | 유지 느슨 폭 조절 |
| `REID_FLOOR` | 0.55 | 0.55 | 0.55 | | 진입 ReID 하한 (고정) |
| `KEEP_REID_FLOOR` | 없음 | 0.45 (신규) | 0.52 | | 유지 중 ReID 하한 — 타인 차단 핵심 |
| `COLOR_FLOOR` | 0.15 | 0.15 | 0.15 | | 색상 하한 (유지 중 미적용) |
| `LOST_MAX` | 20 | 35 | 30 | | 유실 유예 프레임 |
| `MIN_CONFIRM_FRAMES` | 3 | 4 | 4 | | 진입 확인 프레임 |
| `SEARCH_CONFIRM_FRAMES` | 3 | 3 | 3 | | 재획득 확인 프레임 (고정) |
...
- **2차** — 타인 오인식 대응: `KEEP_REID_FLOOR` 0.45→0.52, `KEEP_THRESHOLD` 0.56→0.60, `LOST_MAX` 35→30 (유지를 다시 조임). `SEARCH_MATCH_THR`는 0.65가 별로라 0.70으로 복귀.
- **3차** — (다음 실차 테스트 결과로 채울 것)
